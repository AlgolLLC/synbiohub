	...
	db.model.Remote.find({
		...
		if (defaultRemote === null) {
			if (req.method === 'GET') {
				...
				db.model.Remote.findAll(critera).then(benchlingRemotes => {
			...
			fetchSBOLObjectRecursive(uri, graphUri).then((result) => {
				...
				bSeq.folder = remotes[benchlingRemote].defaultFolderId
				bSeq.name = meta.name
				...
				let flattenedAnnotations = flattenSequenceAnnotations(componentDefinition);

				flattenedAnnotations.forEach(sa => {
					var annotation = {};
				...
				const remoteConfig = remotes[benchlingRemote];
